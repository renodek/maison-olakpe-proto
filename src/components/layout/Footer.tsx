'use client';

import Link from 'next/link';
import {
  Send,
  ArrowUpRight,
} from 'lucide-react';

const footerLinks = {
  navigation: [
    { href: '/catalogue', label: 'Catalogue' },
    { href: '/catalogue?categorie=epices', label: 'Épices' },
    { href: '/catalogue?categorie=parfums', label: 'Parfums' },
    { href: '/catalogue?categorie=cosmetiques', label: 'Cosmétiques' },
    { href: '/recettes', label: 'Recettes' },
  ],
  service: [
    { href: '/contact', label: 'Contactez-nous' },
    { href: '/faq', label: 'FAQ' },
    { href: '/cgv', label: 'CGV' },
    { href: '/mentions-legales', label: 'Mentions légales' },
  ],
};

const socialLinks = [
  {
    name: 'Facebook',
    href: '#',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  {
    name: 'Instagram',
    href: '#',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
      </svg>
    ),
  },
];

export default function Footer() {
  return (
    <footer className="bg-nuit text-creme/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Marque */}
          <div>
            <Link href="/" className="flex items-center gap-2 mb-4 group">
              <div className="w-10 h-10 bg-gradient-to-br from-or to-terre rounded-lg flex items-center justify-center group-hover:scale-105 transition-transform">
                <span className="font-serif font-bold text-creme text-xl">MO</span>
              </div>
              <span className="font-serif font-bold text-creme text-lg group-hover:text-or transition-colors">
                Maison Olakpé
              </span>
            </Link>
            <p className="text-sm leading-relaxed text-creme/60">
              Épices artisanales, parfums d&apos;exception et cosmétiques naturels.
              L&apos;excellence des traditions du monde au service de vos sens.
            </p>
            <div className="flex gap-3 mt-4">
              {socialLinks.map((social) => (
                <a
                  key={social.name}
                  href={social.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-full bg-creme/10 hover:bg-or flex items-center justify-center text-creme/70 hover:text-creme hover:scale-110 active:scale-90 transition-all"
                  aria-label={social.name}
                >
                  {social.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Navigation */}
          <div>
            <h3 className="font-display text-xs uppercase tracking-widest text-or mb-4">
              Navigation
            </h3>
            <ul className="space-y-2 text-sm">
              {footerLinks.navigation.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="hover:text-or transition-colors inline-flex items-center gap-1 group"
                  >
                    {link.label}
                    <ArrowUpRight className="w-3 h-3 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Service Client */}
          <div>
            <h3 className="font-display text-xs uppercase tracking-widest text-or mb-4">
              Service Client
            </h3>
            <ul className="space-y-2 text-sm">
              {footerLinks.service.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="hover:text-or transition-colors inline-flex items-center gap-1 group"
                  >
                    {link.label}
                    <ArrowUpRight className="w-3 h-3 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Newsletter */}
          <div>
            <h3 className="font-display text-xs uppercase tracking-widest text-or mb-4">
              Newsletter
            </h3>
            <p className="text-sm text-creme/60 mb-4">
              Recevez nos recettes exclusives et offres privilégiées.
            </p>
            <form
              onSubmit={(e) => e.preventDefault()}
              className="flex gap-2"
            >
              <input
                type="email"
                placeholder="votre@email.com"
                className="flex-1 bg-creme/10 border border-creme/20 rounded-lg px-3 py-2 text-sm text-creme placeholder:text-creme/30 focus:outline-none focus:border-or focus:ring-1 focus:ring-or/30 transition-all"
              />
              <button
                type="submit"
                className="bg-or hover:bg-or-dark text-creme px-4 py-2 rounded-lg text-sm font-display font-bold uppercase tracking-wider hover:scale-105 active:scale-95 transition-all"
              >
                <Send className="w-4 h-4" />
              </button>
            </form>
          </div>
        </div>

        {/* Barre inférieure */}
        <div className="border-t border-creme/10 mt-10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-creme/40">
          <p>© 2026 Maison Olakpé – Tous droits réservés</p>
          <p>Site prototype – Données et images non contractuelles</p>
        </div>
      </div>
    </footer>
  );
}
